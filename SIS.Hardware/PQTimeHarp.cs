...
namespace SIS.Hardware
{
    ...
    public class PQTimeHarp : TimeHarpDLL
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="PQTimeHarp"/> class. 
        ...
        /// The Time Harp measurement/acquisition mode. There are two possible modes: There are two possible modes: 0 - one-time histogramming and TTTR modes; 1 - continuous mode
        /// </param>
        /// <param name="__iCFDDiscrMin">
        /// The CFD level in [mV] of Time Harp (min value = TimeHarpDefinitions.DISCRMIN; max value = TimeHarpDefinitions.DISCRMAX)
        /// </param>
        /// <param name="__iCFDZeroCross">
        /// The CFD zero cross level in [mV] of Time Harp (min value = TimeHarpDefinitions.ZCMIN; max value = TimeHarpDefinitions.ZCMAX)
        /// </param>
        /// <param name="__iSyncLevel">
        /// The Sync level in [mV] of Time Harp (min value = TimeHarpDefinitions.SYNCMIN; max value = TimeHarpDefinitions.SYNCMAX)
        /// </param>
        /// <param name="__iRangeCode">
        /// The range code, used in order to set up the time resolution of Time Harp (min value = 0; max value = TimeHarpDefinitions.RANGES - 1)
        /// </param>
        /// <param name="__iOffset">
        /// The Time Harp board offset value in [ns], value is approximation of the desired offset (in steps of 2.5ns); (min value = TimeHarpDefinitions.OFFSETMIN; max value = TimeHarpDefinitions.OFFSETMAX)
        /// </param>
        /// <param name="__iMarkerEdge">
        /// Marker 0, set the active TTL edge: 0 - falling edge or 1 - rising edge
        /// </param>
        /// <param name="__iGlobalTTTRBufferSize">
        /// global TTTR buffer size in multiples of Time Harp's Half FiFo Size (see TimeHarpDefinitions.DMABLOCKSZ)
        /// </param>
        /// <param name="__iLinePTTTRBufferSize">
        ...
        /// </summary>
        public int GlobalTTTRBufferSize
        {
            // set or get property value
            get
            {
                return m_iGlobalTTTRBufferSize;
            }

            set
            {
                m_iGlobalTTTRBufferSize = this.GetGlobalTTTRBufferSize(value);
            }
        }
        ...
        public bool IsMeasurementRunning
        {
            ...
            get
            {
                return this.m_IsMeasurementRunning;
            }
        ...
        /// </summary>
        public int LinePTTTRBufferSize
        {
            // set or get property value
            get
            {
                return m_iLinePTTTRBufferSize;
            }

            set
            {
                m_iLinePTTTRBufferSize = this.GetLinePTTTRBufferSize(value);
            }
        }
        ...
        public int TotalSamplesAcquired
        {
            get
            {
                return ScanStatus.TotalPixelsRead;
            }
        ...
        /// </summary>
        /// <param name="__ui32TTTRValue">
        /// 32 bit unsigned integer - a single data record from Time Harp buffer.
        /// </param>
        /// <param name="__PTTTRRecord">
        /// a single ProcessedTTTRecord structure.
        /// </param>
        public static void ConvertSingleTTTRtoPTTTR(uint __ui32TTTRValue, ref ProcessedTTTRecord __PTTTRRecord)
        {
            ...
            {
                    // if Valid = 0 (special event occurred), otherwise if Valid = 1 (photon arrival event occurred)
                case 0:
                    {
                ...
                case 1:
                    {
                        // it makes sure to set the correct values of some variables on photon arrival
        ...
        /// 32 bit unsigned integer - a single data record from Time Harp buffer.
        /// </param>
        /// <param name="__iOverflow">
        /// Overflow flag. Keeps track of the number of overflow events in the Time Harp data buffer.
        /// </param>
        /// <param name="__PTTTRRecord">
        /// a single ProcessedTTTRecord structure.
        /// </param>
        public static void ConvertSingleTTTRtoTrueTimeTagPTTTR(
            uint __ui32TTTRValue, 
            ref int __iOverflow, 
            ref ProcessedTTTRecord __PTTTRRecord)
        {
                ...
                case 0:
                    {
                        ...
                        if (__PTTTRRecord.DataOverflow == 1)
                        {
                            // if DataOverflow = 0 (no overflow of __PTTTRRecord.TimeTag), DataOverflow = 1 (overflow of __PTTTRRecord.TimeTag occurred)
        ...
        /// </summary>
        /// <param name="__indexTTTRBufferLowerBound">
        /// the lower bound for the global TTTR buffer (from where the conversion of TTTR into PTTTR starts).
        /// </param>
        /// <param name="__indexTTTRBufferUpperBound">
        /// the upper bound for the global TTTR buffer (up to where the conversion of TTTR into PTTTR finishes).
        /// </param>
        /// <param name="__ui32TTTRBuffer">
        /// Array of TTTR raw records with the same format as in the Time Harp FiFo buffer.
        /// </param>
        /// <param name="__iOverflow">
        /// Overflow flag. Keeps track of the number of overflow events in the Time Harp data buffer.
        /// </param>
        /// <param name="__iPTTTRBufferOffset">
        /// Offset for PTTTR buffer - we may have records from previous processing.
        /// </param>
        /// <param name="__PTTTRBuffer">
        /// Array that will hold the processed TTTR records.
        /// </param>
        public static void ConvertTTTRBufferToPTTTRBuffer(
            int __indexTTTRBufferLowerBound, 
            int __indexTTTRBufferUpperBound, 
            uint[] __ui32TTTRBuffer, 
            ref int __iOverflow, 
            int __iPTTTRBufferOffset, 
            ProcessedTTTRecord[] __PTTTRBuffer)
        {
            ...
            for (int i = __indexTTTRBufferLowerBound; i < __indexTTTRBufferUpperBound; i++)
            {
                ...
                switch (__PTTTRBuffer[j].Valid)
                {
                    ...
                    case 0:
                        {
                            // Valid = 0 (external trigger or overflow event occurred)
                            if (__PTTTRBuffer[j].DataOverflow == 1)
                            {
                                // if DataOverflow = 0 (no overflow of _PTTTRBuffer[j].TimeTag), DataOverflow = 1 (overflow of _PTTTRBuffer[j].TimeTag occurred)
                                __iOverflow++; // at overflow event, increase the overflow flag   
                            }
        ...
        /// </summary>
        /// <param name="__sInputFile">
        /// The name of the input TTTR file.
        /// </param>
        /// <param name="__sOutputFile">
        ...
        public static void ConvertTTTRFileToASCIIFile(string __sInputFile, string __sOutputFile)
        {
            ...
            {
                throw new Exception(
                    "TimeHarpLib: exception --> " + "the file: " + __sInputFile
                    + " could not be opened by ConvertTTTRFileToASCIIFile() function!" + " Exception Message: "
                    + ex.Message);
            }
            ...
            {
                throw new Exception(
                    "TimeHarpLib: exception --> " + "the file: " + __sOutputFile
                    + " could not be created and opened by ConvertTTTRFileToASCIIFile() function!"
            ...
            _swOutputFile.WriteLine(
                "Identifier       : {0}", 
                (new String(Files.TTTRFileHeader.TextHeader.Ident)).Trim('\0'));
            _swOutputFile.WriteLine(
                "Format Version   : {0}", 
                (new String(Files.TTTRFileHeader.TextHeader.FormatVersion)).Trim('\0'));
            _swOutputFile.WriteLine(
                "Creator Name     : {0}", 
                (new String(Files.TTTRFileHeader.TextHeader.CreatorName)).Trim('\0'));
            _swOutputFile.WriteLine(
                "Creator Version  : {0}", 
                (new String(Files.TTTRFileHeader.TextHeader.CreatorVersion)).Trim('\0'));
            _swOutputFile.WriteLine(
                "Time of Creation : {0}", 
                (new String(Files.TTTRFileHeader.TextHeader.FileTime)).Trim('\0'));
            _swOutputFile.WriteLine(
                "File Comment     : {0}", 
                (new String(Files.TTTRFileHeader.TextHeader.CommentField)).Trim('\0'));
            ...
            _swOutputFile.WriteLine(
                "HardwareIdent    : {0}", 
                (new String(Files.TTTRFileHeader.BoardHeader.HardwareIdent)).Trim('\0'));
            _swOutputFile.WriteLine(
                "HardwareVersion  : {0}", 
                (new String(Files.TTTRFileHeader.BoardHeader.HardwareVersion)).Trim('\0'));
            ...
            if (Files.TTTRFileHeader.TTTRHeader.SpecialHeaderLength * 4
                == TimeHarpDefinitions.SizeTTTRCustomSISFileHeader)
            {
                _swOutputFile.WriteLine(string.Empty);
                _swOutputFile.WriteLine(
                    "Custom SIS header (version = {0}):", 
                    (new String(Files.TTTRFileHeader.CustomSISHeader.Version)).Trim('\0'));
                ...
                _swOutputFile.WriteLine(
                    "X Image Width    : {0} [px]", 
                    Files.TTTRFileHeader.CustomSISHeader.ImageWidthPx);
                _swOutputFile.WriteLine(
                    "Y Image Height   : {0} [px]", 
                    Files.TTTRFileHeader.CustomSISHeader.ImageHeightPx);
                _swOutputFile.WriteLine(
                    "Z Image Depth    : {0} [px]", 
                    Files.TTTRFileHeader.CustomSISHeader.ImageDepthPx);
                ...
                _swOutputFile.WriteLine(
                    "Frame Time Out   : {0} [ms]", 
                    Files.TTTRFileHeader.CustomSISHeader.FrameTimeOut);
                ...
                _swOutputFile.WriteLine(
                    "Galvo Magnification Objective: {0}", 
                    Files.TTTRFileHeader.CustomSISHeader.GalvoMagnificationObjective);
                _swOutputFile.WriteLine(
                    "Galvo Scan Lens Focal Length : {0} [mm]", 
                    Files.TTTRFileHeader.CustomSISHeader.GalvoScanLensFocalLength);
                _swOutputFile.WriteLine(
                    "Galvo Max Range Angle        : {0} [degrees]", 
                    Files.TTTRFileHeader.CustomSISHeader.GalvoRangeAngleDegrees);
                _swOutputFile.WriteLine(
                    "Galvo Max Range Angle        : {0} [integer]", 
                    Files.TTTRFileHeader.CustomSISHeader.GalvoRangeAngleInt);
            }
            ...
            uint _ui32TTTRSingleRecord = 0U; // the TTTR record is actually a unsigned 32bit integer
            ...
            for (int i = 0; i < Files.TTTRFileHeader.TTTRHeader.NumberOfRecords; i++)
            {
                    ...
                    case 0:
                        {
                            ...
                            if (_PTTTRecord.DataMarker > 0)
                            {
                                ...
                                _swOutputFile.WriteLine(
                                    ...
                                    i, 
                                    _ui32TTTRSingleRecord.ToString("X8"), 
                                    " ", 
                                    _PTTTRecord.TimeTag, 
                                    _lTrueTimeTag * 100e-9, 
                                    _PTTTRecord.DataMarker);
                            }

                            if (_PTTTRecord.DataOverflow == 1)
                            {
        ...
        /// The minimum gating time in milliseconds
        /// </param>
        /// <param name="__dGatingTimeMaxMillisec">
        /// The maximum gating time in milliseconds (only photons between min and max gating time will pass the gate, i.e. will have contribution to the image)
        /// </param>
        /// <param name="__sInputFile">
        /// The name of the input TTTR file
        /// </param>
        /// <param name="__iRecordsOffset">
        /// The specific TTTR records number from where we start reading
        /// </param>
        /// <param name="__dXScanSizeNm">
        /// We return here the width in nm of the extracted image
        /// </param>
        /// <param name="__dYScanSizeNm">
        /// We return here the height in nm of the extracted image
        /// </param>
        /// <param name="__iXImageWidth">
        /// We return here the width in pixels of the extracted image
        /// </param>
        /// <param name="__iYImageHeight">
        /// We return here the height in pixels of the extracted image
        /// </param>
        /// <param name="__ui32SingleFramePixelBuffer">
        /// The extracted image will be stored and returned in this buffer
        /// </param>
        public static void ExtractImageFromTTTRFile(
            bool __bIsToApplyTimeGating, 
            double __dGatingTimeMinMillisec, 
            double __dGatingTimeMaxMillisec, 
            string __sInputFile, 
            ref int __iRecordsOffset, 
            ref double __dXScanSizeNm, 
            ref double __dYScanSizeNm, 
            ref int __iXImageWidth, 
            ref int __iYImageHeight, 
            ref uint[] __ui32SingleFramePixelBuffer)
        {
            ...
            catch (Exception ex)
            {
                throw new Exception(
                    "TimeHarpLib: exception --> " + "the file: " + __sInputFile
                    + " could not be opened by ConvertTTTRFileToBitmapImages() function!" + " Exception Message: "
                    + ex.Message);
            }
            ...
            for (int i = 0; i < (Files.TTTRFileHeader.TTTRHeader.NumberOfRecords / TimeHarpDefinitions.DMABLOCKSZ); i++)
            {
                // Read TTTR records in multiple of TimeHarpDefinitions.DMABLOCKSZ
                // Get records (in TimeHarpDefinitions.DMABLOCKSZ size) into the TTTR buffer                
                for (int j = 0; j < TimeHarpDefinitions.DMABLOCKSZ; j++)
                {
                ...
                {
                    // loop until we process the entire TTTR buffer
                    // Process TTTR buffer and extract pixels from photon events                    
                    ExtractPhotonEventsFromTTTRBuffer(
                        _ui32TTTRBuffer, 
                        ref _indexTTTRBufferLowerBound, 
                        _indexTTTRBufferUpperBound, 
                        ref _bFrameReady, 
                        _ui32FramePixelBuffer[0]);
            ...
            {
                for (int j = 0;
                     j < (Files.TTTRFileHeader.TTTRHeader.NumberOfRecords % TimeHarpDefinitions.DMABLOCKSZ);
                     j++)
                {
                    // fill the _ui32TTTRBuffer[] buffer array with records
                    _ui32TTTRBuffer[j] = _brInputFile.ReadUInt32();
            ...
            if (_bFrameReady || ExternalMarkers.FrameCount > 0)
            {
                // the case of complete or incomplete frame - note that when the frame is incomplete we nevertheless return what's left in pixel buffer (due to possible FiFo buffer overrun, which may wipe reference line markers)
            ...
            {
                // case no frame or frame marker found - we return zeros and nulls
                __dXScanSizeNm = 0.0;
                __dYScanSizeNm = 0.0;
                __iXImageWidth = 0;
                __iYImageHeight = 0;
        ...
        /// </summary>
        /// <param name="__ui32TTTRBuffer">
        /// An array of UInt32 numbers - the raw TTTR records.
        /// </param>
        /// <param name="__indexTTTRBufferLowerBound">
        /// The index of the TTTR buffer from which we start to process the buffer.
        /// </param>
        /// <param name="__indexTTTRBufferUpperBound">
        /// The maximum index up to which we can process the current TTTR buffer.
        /// </param>
        /// <param name="_bFrameReady">
        /// Indicates if the frame is ready - i.e. all photons within a frame are converted into pixels.
        /// </param>
        /// <param name="__ui32FramePixelBuffer">
        /// The pixel buffer - the binned photon events go in this array (the buffer represent a scanned frame or part of a scanned frame).
        /// </param>
        public static void ExtractPhotonEventsFromTTTRBuffer(
            uint[] __ui32TTTRBuffer, 
            ref int __indexTTTRBufferLowerBound, 
            int __indexTTTRBufferUpperBound, 
            ref bool _bFrameReady, 
            uint[] __ui32FramePixelBuffer)
        {
            ...
            // Process TTTR buffer and extract pixels from photon events
            while (__indexTTTRBufferLowerBound < __indexTTTRBufferUpperBound)
            {
                // loop until we process the entire TTTR buffer
                // Convert raw TTTR records to processed PTTTR records
                ConvertSingleTTTRtoTrueTimeTagPTTTR(
                    __ui32TTTRBuffer[__indexTTTRBufferLowerBound], 
                    ref _iOverflow, 
                    ref _PTTTRecord); // extract information form a single 32 bits integer data record 
                ...
                {
                    // handles FiFo buffer overrun by resetting frame pixel buffer
                    switch (_PTTTRecord.Valid)
                    {
                            // Valid = 0 (external trigger or overflow event), Valid = 1 (means photon arrival event)
                        case 0:
                            {
                                // Valid = 0 (external trigger or overflow event occurred)
                                if (_PTTTRecord.DataMarker == ScanStatus.FrameMarker
                                    && ExternalMarkers.LineCount < _iYHeight)
                                {
                ...
                if (!ExternalMarkers.FrameMarker.Found)
                {
                    // find the reference marker for frame
                    switch (_PTTTRecord.Valid)
                    {
                            // Valid = 0 (external trigger or overflow event), Valid = 1 (means photon arrival event)
                        case 0:
                ...
                else if (ExternalMarkers.FrameMarker.Found && !ExternalMarkers.LineMarker.Found)
                {
                    // find the reference marker for next line so that we can calculate the time per pixel
                    ...
                    switch (_PTTTRecord.Valid)
                    {
                        ...
                        case 1:
                            {
                                // Valid = 1 (photon arrival event occurred)
                                if (ScanStatus.LinePTTTRBufferIndex < ScanStatus.LinePTTTRBuffer.Length)
                                {
                                ...
                                {
                                    // line TTTR buffer overrun - it happens if we have too many photon events
                                    ...
                                    _logger.Debug(
                                        "Time Harp: exception, ScanStatus.LinePTTTRBuffer overrun by function ExtractPhotonEventsFromTTTRBuffer(): to fix the problem edit the source code and increase the size of this buffer!");
                                }
                ...
                if (ExternalMarkers.FrameMarker.Found && ExternalMarkers.LineMarker.Found)
                {
                    // Convert the line PTTTR buffer to pixels array - the conversion is line by line, i.e. first we get/fill one line and then we convert it to pixels  
                    ...
                    PhotonEventsToPixelBuffer(
                        _iTimePPixel, 
                        _iXWidth, 
                        _iPixelCount, 
                        ref _iPixelCounter, 
                        ref _bFrameReady, 
                        _ui32LinePixelBuffer, 
                        __ui32FramePixelBuffer); // photon counts to pixels
        ...
        /// Binning of photons - process the photon events and convert them to pixel data. Store pixels in the respective frame pixel buffer.
        /// </summary>
        /// <param name="__iTimePPixel">
        /// The time per pixel in terms of channel number (channel width is 100e-9[s]).
        /// </param>
        /// <param name="__iXWidth">
        ...
        /// </param>
        /// <param name="__iPixelCounter">
        /// The pixels counter. Keeps track of the current pixel to process.
        /// </param>
        /// <param name="__bFrameReady">
        /// The __b Frame Ready.
        /// </param>
        /// <param name="__ui32LinePixelBuffer">
        /// The pixel buffer - the binned photon events go in this array (the buffer represent a scanned frame or part of the scanned frame).
        /// </param>
        /// <param name="__ui32FramePixelBuffer">
        /// The pixel buffer - the binned photon events go in this array (the buffer represent a scanned frame or part of the scanned frame).
        /// </param>
        public static void PhotonEventsToPixelBuffer(
            int __iTimePPixel, 
            int __iXWidth, 
            int __iPixelCount, 
            ref int __iPixelCounter, 
            ref bool __bFrameReady, 
            uint[] __ui32LinePixelBuffer, 
            uint[] __ui32FramePixelBuffer)
        {
            ...
            if (__iPixelCounter < __iPixelCount)
            {
                ...
                switch (ScanStatus.IsToApplyTimeGating)
                {
                    case true:
                        {
                            // the case of time gating
                            while (i < ScanStatus.LinePTTTRBufferIndex)
                            {
                                ...
                                if (_iTimeChannel <= __iTimePPixel)
                                {
                                    ...
                                    if (ScanStatus.LinePTTTRBuffer[i].Channel < ScanStatus.GatingTimeMinChannel
                                        && ScanStatus.LinePTTTRBuffer[i].Channel > ScanStatus.GatingTimeMaxChannel)
                                    {
                                        // check if the photon passes the gate
                                        __ui32LinePixelBuffer[_iLinePixelCounter] += 1U; // bin photons
                                    }
                                ...
                                else
                                {
                                    ...
                                    if (_iLinePixelCounter >= __iXWidth)
                                    {
                                        // this condition will discard photons which may exceed the the channel number of the current line (this is necessary due to rounding down to nearest integer when calculating Time Per Pixel)
                    ...
                    case 0:
                        {
                            ...
                            for (int indexPixel = 0; indexPixel < __iXWidth; indexPixel++)
                            {
            ...
            if (__iPixelCounter >= __iPixelCount)
            {
                // the case when the frame is ready
                ...
                _logger.Debug(
                    "Time Harp: number scanned lines per frame found # " + ExternalMarkers.LineCount.ToString());
        ...
        /// </summary>
        /// <param name="__sInputFile">
        /// The name of the input TTTR file to be read.
        /// </param>
        /// <param name="__TTTRFileHeader">
        /// A TTTR file header structure where the info from TTTR file header will be stored.
        /// </param>
        public static void ReadTTTRHeaderFromFile(
            string __sInputFile, 
            ref TimeHarpDefinitions.StructTTTRFileHeader __TTTRFileHeader)
        {
            ...
            catch (Exception ex)
            {
                throw new Exception(
                    "TimeHarpLib: exception --> " + "the file: " + __sInputFile
                    + " could not be opened by ReadTTTRHeaderFromFile() function!" + "Exception Message: " + ex.Message);
            }
        ...
        /// </summary>
        /// <param name="__sOutputFile">
        /// The name of the output image file(s).
        /// </param>
        /// <param name="__iXWidth">
        /// The number of pixels per line.
        /// </param>
        /// <param name="__iYHeight">
        /// The number of lines per frame.
        /// </param>
        /// <param name="__ui32FramePixelBuffer">
        /// The pixel buffer - the binned photon events go in this array (the buffer represent a scanned frame or part of a scanned frame).
        /// </param>
        /// <param name="__iFrameCounter">
        /// The starting number of the saved bitmap file (in case you want to save more then one file with the same name, use this to distinguish between images).
        /// </param>
        public static void SaveFramePixelBufferAsBitmapImage(
            string __sOutputFile, 
            int __iXWidth, 
            int __iYHeight, 
            uint[] __ui32FramePixelBuffer, 
            int __iFrameCounter)
        {
            ...
            BitmapData bitmapData = bitmapImage2D.LockBits(
                new System.Drawing.Rectangle(0, 0, __iXWidth, __iYHeight), 
            ...
            {
                throw new Exception(
                    "Time Harp: exception --> " + "the bitmap image file: " + __iFrameCounter.ToString() + "."
                    + __sOutputFile + " could not be saved by ConvertTTTRFileToBitmapImages() function!"
                    + " Exception Message: " + ex.Message);
            }
        }
        ...
        /// </summary>
        /// <param name="__bwOutputFile">
        /// The binary writer stream that takes care to write the binary data to file.
        /// </param>
        /// <param name="__iFileOffsetForNumberRecords">
        /// The byte offset relative to the origin of the TTTR file.
        /// </param>
        /// <param name="__iRecordsCount">
        /// The current number of records that have been written to a TTTR file.
        /// </param>
        public static void WriteTTTRNumberOfRecordsToFile(
            BinaryWriter __bwOutputFile, 
            int __iFileOffsetForNumberRecords, 
            int __iRecordsCount)
        {
        ...
        public void Initialize()
        {
            ...
            if (!this.m_bIsInitialized)
            {
                ...
                if (_sLibraryVersion != TimeHarpDefinitions.TargetLibVersion)
                {
                    // check if we match the installed DLL version and the supported in SIS one
                ...
                if (this.m_iErrorCode >= (int)TimeHarpDefinitions.THError.ERROR_NONE)
                {
                    ...
                    _logger.Info(
                        string.Format("Time Harp: sync rate = {0} Hz ({1} MHz)", _iSyncRate, ((double)_iSyncRate) / 1e6));

                    int _iCountRate = this.CountRate; // get the count rate in counts/sec
                    _logger.Info(
                        string.Format(
                            "Time Harp: count rate = {0} Cps ({1} Mcps)", 
                            _iCountRate, 
                            ((double)_iCountRate) / 1e6));
                }
                else
                {
        ...
        public void RunAPDAcquisition()
        {
            ...
            if (ThreadsStatus.APDReadThread.Thread.IsAlive)
            {
                ThreadsStatus.APDReadThread.Thread.Join(); // wait for the thread to finish
            }

            if (ThreadsStatus.APDSaveThread.Thread.IsAlive)
            {
                ThreadsStatus.APDSaveThread.Thread.Join(); // wait for the thread to finish          
            }
        }
        ...
        /// External frame marker that marks a beginning of frame in the Time Harp data stream
        /// </param>
        /// <param name="__iLineMarker">
        /// External line marker that marks a beginning of line in the Time Harp data stream
        /// </param>
        /// <param name="__dTimePPixelMillisec">
        /// The time per pixel in [ms] - we use it just for info (the real pix time is calculated from the line markers)
        /// </param>
        /// <param name="__iXImageWidthPx">
        /// The number of pixels per line
        /// </param>
        /// <param name="__iYImageHeightPx">
        /// The number of lines per frame
        /// </param>
        /// <param name="__iZImageDepthPx">
        /// The number of images per stack
        /// </param>
        /// <param name="__dXScanSizeNm">
        /// The physical size of the scan along X in[nm]
        /// </param>
        /// <param name="__dYScanSizeNm">
        /// The physical size of the scan along Y in[nm]
        /// </param>
        /// <param name="__dZScanSizeNm">
        /// The physical size of the scan along Z in[nm]
        /// </param>
        /// <param name="__dInitXNm">
        /// The initial scan offset along X in [nm]
        /// </param>
        /// <param name="__dInitYNm">
        /// The initial scan offset along Y in [nm]
        /// </param>
        /// <param name="__dInitZNm">
        /// The initial scan offset along Z in [nm]
        /// </param>
        /// <param name="__iXOverScanPx">
        /// Overscan size in [px] along X
        /// </param>
        /// <param name="__iYOverScanPx">
        /// Overscan size in [px] along Y
        /// </param>
        /// <param name="__iZOverScanPx">
        /// Overscan size in [px] along Z
        /// </param>
        /// <param name="__iSISChannels">
        /// The number of channels that SIS shows/processes at the same time
        /// </param>
        /// <param name="__dMagnificationObjective">
        /// The magnification of the objective
        /// </param>
        /// <param name="__dScanLensFocalLength">
        /// The focal length of the scan lens
        /// </param>
        /// <param name="__dRangeAngleDegrees">
        /// The +/- of the maximum useful angle in [degrees] the galvo can scan
        /// </param>
        /// <param name="__dRangeAngleInt">
        /// The +/- of the maximum useful angle in [terms of integer] the galvo can scan - this is the value that the galvo's controller takes for an angle
        /// </param>
        /// <param name="__bIsToApplyTimeGating">
        /// Indicates whether we want to do time gating or not (true = do gating; false = do not perform gating)
        /// </param>
        /// <param name="__dGatingTimeMinMillisec">
        /// The  min gating time in milliseconds
        /// </param>
        /// <param name="__dGatingTimeMaxMillisec">
        /// The max gating time in milliseconds
        /// </param>
        /// <param name="__iTypeOfScan">
        /// The type of scan mode (0 - unidirectional, 1 - bidirectional, 2 - line scan, 3 - point scan)
        /// </param>
        /// <param name="__iFrameTimeOut">
        /// The frame time out - the time after which a frame a part of the already processed frame will be returned
        /// </param>
        /// <param name="__iFiFoTimeOut">
        /// The Time Harp's FiFo time out - the time after which the FiFo will be read no matter if it is half full or not
        /// </param>
        /// <param name="__iMeasurementMode">
        ...
        /// </param>
        /// <param name="__bSaveTTTRData">
        /// Indicates if we want to save the TTTR data to file or not (true = save it, false = do not save)
        /// </param>
        /// <param name="__sTTTRFile">
        /// The name of the binary TTTR file where the acquired records will be stored
        /// </param>
        ...
        public void StartAPDAcquisition()
        {
            ...
            while (!ThreadsStatus.APDReadThread.Thread.IsAlive)
            {
                // wait until the thread gets alive
        ...
        /// </summary>
        /// <param name="__TTTRBuffer">
        /// The TTTR buffer. It is an array of UInt32 numbers - represents the raw Time Harp data records.
        /// </param>
        /// <param name="__indexLowerTTTRBuffer">
        /// The lower index of the TTTR data buffer from where we start the saving of data.
        /// </param>
        /// <param name="__indexUpperTTTRBuffer">
        /// The upper index of the TTTR data buffer up to which we save data.
        /// </param>
        /// <param name="__sOutputFile">
        /// The name of the output TTTR file.
        /// </param>
        /// <param name="__TTTRBuffer">
        /// The TTTR buffer. It is an array of UInt32 numbers - represents the raw Time Harp data records.
        /// </param>
        public void WriteTTTRDataToFile(
            uint[] __TTTRBuffer, 
            int __indexLowerTTTRBuffer, 
            int __indexUpperTTTRBuffer, 
            string __sOutputFile)
        {
            ...
            string _strCurrentFileName = string.Empty;

            if (Files.isTTTRFileFirstWrite)
            {
                // if it is the first time we store a buffer to a file, reset some parameters and create the TTTR file and write TTTR header to file
                ...
                try
                {
                    ...
                    WriteTTTRBufferToFile(
                        _bwOutputFile, 
                        ref Files.RecordsCount, 
                        __indexLowerTTTRBuffer, 
                        __indexUpperTTTRBuffer, 
                        __TTTRBuffer); // TTTR buffer to file
                    WriteTTTRNumberOfRecordsToFile(
                        _bwOutputFile, 
                ...
                catch (Exception ex)
                {
                    ...
                    {
                        _logger.Error(
                            "Time Harp: reporting error --> the file " + _strCurrentFileName
                            + " could not be created by WriteTTTRDataToFile() function!" + " Exception Message: "
                            + ex.Message);
                        this.ErrorOccurred(this, new EventArgs());
                    }
                }
            ...
            {
                // if we write not for first time, it means we have to only append the binary data to the TTTR file
                try
                {
                    // create and open the file for writing
                    if (Files.RecordsCount > Files.MAXIMUM_TTTR_RECORDS)
                    {
                    ...
                    {
                        // open the previous file and append the buffer data to it
                        _strCurrentFileName = Path.ChangeExtension(
                            __sOutputFile, 
                            "." + Files.FileCounter.ToString() + ".t3r");
                    ...
                    WriteTTTRNumberOfRecordsToFile(
                        _bwOutputFile, 
                        TimeHarpDefinitions.SizeTTTRDefaultFileHeader - 8, 
                        Files.RecordsCount); // update the number of records written to the TTTR file so far
                }
                catch (Exception ex)
                {
                    // Shutdown();  //close Time harp 200 device
        ...
        /// </param>
        /// ///
        public void WriteTTTRHeaderToFile(
            BinaryWriter __bwOutputFile, 
            ref TimeHarpDefinitions.StructTTTRFileHeader __TTTRFileHeader)
        {
            ...
            __TTTRFileHeader.TextHeader.FileTime = StringToCharArray(
            ...
            __TTTRFileHeader.CustomSISHeader.Version =
                StringToCharArray(TimeHarpDefinitions.VersionTTTRCustomSISFileHeader, 8);
        ...
        /// </summary>
        /// <param name="__InputString">
        /// The input string which will be converted to array of chars
        /// </param>
        /// <param name="__iCharLength">
        /// The length of the returned array of chars.
        /// </param>
        /// <returns>
        /// char[]: returns an array of chars with a length defined by __iCharLength
        /// </returns>
        private static char[] StringToCharArray(string __InputString, int __iCharLength)
        {
            ...
            if (_iCharBufferLength > __iCharLength)
            {
            ...
            {
                // copy chars to the array which will be returned
                _charData[i] = _charBuffer[i];
            }

            for (int i = _iCharBufferLength; i < __iCharLength; i++)
            {
                // fill the rest with null character
                _charData[i] = '\0';
            }
        ...
        /// </param>
        /// <param name="__iRecordsCount">
        /// The current number of TTTR data records.
        /// </param>
        /// <param name="__indexLowerTTTRBuffer">
        /// The lower index of the TTTR data buffer from where we start the saving of data.
        /// </param>
        ...
        /// The TTTR buffer. It is an array of UInt32 numbers - represents the raw Time Harp data records.
        /// </param>
        private static void WriteTTTRBufferToFile(
            BinaryWriter __bwOutputFile, 
            ref int __iRecordsCount, 
            int __indexLowerTTTRBuffer, 
            int __indexUpperTTTRBuffer, 
            uint[] __TTTRBuffer)
        {
            // TTTR buffer to file
            for (int i = __indexLowerTTTRBuffer; i < __indexUpperTTTRBuffer; i++)
            {
                // write data to file and reset to zero the data array elements
        ...
        /// </summary>
        /// <param name="__iNumberTTTRBufferToProcess">
        /// The number of the next TTTR buffer that is in a state to be processed.
        /// </param>
        /// <param name="__iNumberOfRecords">
        /// Number of records obtained from the Time Harp TTTR buffer.
        /// </param>
        /// <param name="__iNumberOfRecords1">
        /// Number of records obtained from the Time Harp TTTR buffer 1.
        /// </param>
        /// <param name="__iNumberOfRecords2">
        /// Number of records obtained from the Time Harp TTTR buffer 2.
        /// </param>
        private void AssignIndexesForNextTTTRBuffer(
            ref int __iNumberTTTRBufferToProcess, 
            int __iNumberOfRecords, 
            ref int __iNumberOfRecords1, 
            ref int __iNumberOfRecords2)
        {
            switch (__iNumberTTTRBufferToProcess)
            {
                    // assign few variables so that we flip the TTTR buffers
                case 0:
                    {
        ...
        private void BuildImage()
        {
            ...
            while (_bMeasurementRunning && !_bMesurementToBeStop)
            {
        ...
        private void CheckGlobalTTTROverwrite(int __iNumberOfRecords)
        {
            if (ThreadsStatus.APDSaveThread.Thread.IsAlive)
            {
                // check first if the Save() thread is running
                if ((ScanStatus.SizeGlobalTTTRBuffer < ThreadsStatus.APDSaveThread.IndexTTTRBufferLowerBound)
                    && ((ScanStatus.SizeGlobalTTTRBuffer + __iNumberOfRecords)
                        > ThreadsStatus.APDSaveThread.IndexTTTRBufferLowerBound))
                {
                    // check if there is danger to overwrite an area of the global buffer still in processing by Save() function
                    ...
                    _logger.Warn(
                        "Time Harp: reporting a warning --> in the next step Read() function will overwrite, a still in processing by Save() function, area of the ScanStatus.GlobalTTTRBuffer[]!");
                }
            }

            if (ThreadsStatus.BuildImageThread.Thread.IsAlive)
            {
                // check first if the BuildImage() thread is running
                        ...
                        > ThreadsStatus.BuildImageThread.IndexTTTRBufferLowerBound))
                {
                    ...
                    _logger.Warn(
                        "Time Harp: reporting a warning --> in the next step Read() function will overwrite, a still in processing by BuildImage() function, area of the ScanStatus.GlobalTTTRBuffer[]!");
                }
            }
        }
        ...
        /// </summary>
        /// <param name="__ui32TTTRBuffer">
        /// Array of raw TTTR records from Time Harp TTTR buffer.
        /// </param>
        /// <param name="__iNumberOfRecords">
        /// Number of records obtained from the Time Harp TTTR buffer.
        /// </param>
        private void CopyFiFoToArray(uint[] __ui32TTTRBuffer, int __iNumberOfRecords)
        {
            if ((ScanStatus.SizeGlobalTTTRBuffer + __iNumberOfRecords - 1) < ScanStatus.GlobalTTTRBuffer.Length)
            {
                ...
                Array.Copy(
                    ...
                    0, 
                    ScanStatus.GlobalTTTRBuffer, 
                    ScanStatus.SizeGlobalTTTRBuffer, 
                    __iNumberOfRecords); // copy TTTR buffer to the Global TTTR buffer
                ScanStatus.SizeGlobalTTTRBuffer += __iNumberOfRecords;
            }
            else
            {
                ...
                Array.Copy(
                    ...
                    ScanStatus.GlobalTTTRBuffer, 
                    ScanStatus.SizeGlobalTTTRBuffer, 
                    _iNumberFreeCells); // copy TTTR buffer to the Global TTTR buffer
                ...
                Array.Copy(
                    ...
                    ScanStatus.GlobalTTTRBuffer, 
                    ScanStatus.SizeGlobalTTTRBuffer, 
                    __iNumberOfRecords - _iNumberFreeCells); // copy TTTR buffer to the Global TTTR buffer
        ...
        /// </summary>
        /// <param name="__iGlobalTTTRBufferSize">
        /// The __i Global TTTR Buffer Size.
        /// </param>
        ...
        /// </summary>
        /// <param name="__iLinePTTTRBufferSize">
        /// Line PTTTR buffer size value to assign.
        /// </param>
        ...
        private void Read()
        {
            ...
            while (_bMeasurementRunning && !_bMesurementToBeStop)
            {
                ...
                if (_bFiFoFull)
                {
                    // Handle the FIFO overrun case - basically reset FiFo and tries to continue
                ...
                else if (!_bMeasurementRunning)
                {
                    // Handle the end of measurement case - basically here we finalize the acquiring of the data
                    for (int i = 0; i < (TimeHarpDefinitions.FIFOSIZE / TimeHarpDefinitions.DMABLOCKSZ); i++)
                    {
                        // max one FIFO full
                        // Fetch data records from FIFO                    
                        this.StartDMATransfer(
                            ref _ui32TTTRBuffer, 
                            _ui32TTTRBuffer1, 
                            _ui32TTTRBuffer2, 
                            _iNumberTTTRBufferToProcess, 
                            ref _iNumberOfRecords, 
                            _iNumberOfRecords1, 
                            _iNumberOfRecords2);

                        // Process any TTTR buffer which is ready to be processed
                        if (_iNumberTTTRBufferToProcess > 0)
                        {
                            // check if we really have a buffer to process (must be 1 or 2 for a ready to process buffer)
                        ...
                        if (_iNumberOfRecords < 0)
                        {
                            // if _iNumberOfRecord is less than zero, then an error occurred
                        ...
                        {
                            // check if the DMA transfer was successful (i.e. without errors)
                            _iStatusFlags = this.GetFlags(); // get Time Harp status flags
                            if (!this.IsFIFOEmpty(_iStatusFlags))
                            {
                                // in this case it is a DMA error (because the buffer is not empty)
                                // Shutdown();  //close Time Harp device
                                // throw new Exception("KUL.MDS.Hardware.PQTimeHarp Exception - Time Harp DMA buffer error by function Read()");
                        ...
                        this.AssignIndexesForNextTTTRBuffer(
                            ref _iNumberTTTRBufferToProcess, 
                            _iNumberOfRecords, 
                            ref _iNumberOfRecords1, 
                    ...
                    switch (_iNumberTTTRBufferToProcess)
                    {
                        case 0:
                            {
                                // _iNumberTTTRBufferToProcess = 0, i.e. the first TTTR acquisition
                ...
                else if (_bFiFoHalfFull)
                {
                    // Handle the FIFO half full case - basically here we acquire the data
                ...
                else if (_bFiFoTimeOut && !_bFiFoEmpty)
                {
                    // Handle the case of slow counts rate - process buffer data before FIFO gets half full. Buffer must be not empty in order to process, i.e. _bFiFoEmpty = false
                    // Fetch data records from FIFO                    
                    this.StartDMATransfer(
                        ref _ui32TTTRBuffer, 
                        _ui32TTTRBuffer1, 
            ...
            _logger.Info(
                "Time Harp: FiFo overrun occurred " + ThreadsStatus.APDReadThread.FiFoOverrunCount.ToString() + "x"
                + " during scanning!");
        ...
        /// </summary>   
        private void RequestMeasurementStop()
        {
            // stop Time Harp acquisition by setting IsMesurementToBeStop = True
        ...
        private void Save()
        {
            ...
            while (_bMeasurementRunning && !_bMesurementToBeStop)
            {
                ...
                {
                    // it means the filling of the GlobalTTTRBuffer[] buffer started again so save the chunk up to the end of the buffer and prepare it for the next round of storage iteration
                    this.WriteTTTRDataToFile(
                        ScanStatus.GlobalTTTRBuffer, 
                        _indexTTTRBufferLowerBound, 
        ...
        /// </summary>
        /// <param name="__ui32TTTRBuffer">
        /// Point to the current TTTR buffer to be processed.
        /// </param>
        /// <param name="__ui32TTTRBuffer1">
        /// The TTTR buffer 1 - records from DMA come here or in TTTR buffer 2.
        /// </param>
        /// <param name="__ui32TTTRBuffer2">
        /// The TTTR buffer 2 - records from DMA come here or in TTTR buffer 1.
        /// </param>
        /// <param name="__iNumberTTTRBufferToProcess">
        /// The number of the next TTTR buffer that is in a state to be processed.
        /// </param>
        ...
        private void StartDMATransfer(
            ref uint[] __ui32TTTRBuffer, 
            uint[] __ui32TTTRBuffer1, 
            uint[] __ui32TTTRBuffer2, 
            int __iNumberTTTRBufferToProcess, 
            ref int __iNumberOfRecords, 
            int __iNumberOfRecords1, 
            int __iNumberOfRecords2)
        {
            switch (__iNumberTTTRBufferToProcess)
            {
                case 0:
        ...
        public struct Files
        {
            ...
            /// The ascii file name.
            /// </summary>
            public static string ASCIIFileName = string.Empty; // file name of the output data file
            ...
            /// The tttr file name.
            /// </summary>
            public static string TTTRFileName = string.Empty; // file name of the output data file
        ...
        private struct ExternalMarkers
        {
            ...
            {
                // Frame maker, stores index and time tag of the a frame in the currently processed TTTR buffer
                #region Static Fields
            ...
            {
                // Line maker, stores index and time tag of the a line in the currently processed TTTR buffer
                #region Static Fields
        ...
        private struct ThreadsStatus
        {
            ...
            /// The apd read thread.
            /// </summary>
            public struct APDReadThread
            {
                // keeps track of the thread that spawns Read() function (that fetches the Time Harp FiFo buffer)
            ...
            /// The apd save thread.
            /// </summary>
            public struct APDSaveThread
            {
                // keeps track of the thread that spawns Save() function (that saves the GlobalTTTRBuffer[] buffer to file)                
                #region Static Fields
            ...
            /// <summary>
            /// The build image thread.
            /// </summary>
            public struct BuildImageThread
            {
                // keeps track of the main thread that executes BuildImage() function (that extract pixels from the global buffer)
                #region Static Fields
