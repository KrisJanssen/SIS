// --------------------------------------------------------------------------------------------------------------------
// <copyright company="" file="GraphPane.cs">
//   
// </copyright>
// <summary>
...
namespace ZedGraph.ZedGraph
{
    ...
    public class GraphPane : PaneBase, ICloneable, ISerializable
    {
        ...
        [Description(
            "Determines whether the auto-ranged scale will include all data points" + " or just the visible data points"
            )]
        ...
        public Y2Axis Y2Axis
        {
            get
            {
                return this._y2AxisList[0] as Y2Axis;
            }
        }
        ...
        public YAxis YAxis
        {
            get
            {
                return this._yAxisList[0] as YAxis;
            }
        }
        ...
        public bool FindContainedObjects(RectangleF rectF, Graphics g, out CurveList containedObjs)
        {
            ...
            foreach (CurveItem ci in this.CurveList)
            {
                ...
                {
                    if (ci.Points[i].X > rectF.Left && ci.Points[i].X < rectF.Right && ci.Points[i].Y > rectF.Bottom
            ...
            out int iNearest)
        {
                ...
                else if (curve.IsVisible)
                {
                    ...
                    if (points != null)
                    {
                        for (int iPt = 0; iPt < curve.NPts; iPt++)
                        {
                            ...
                            if (xVal != PointPair.Missing && yVal != PointPair.Missing)
                            {
                                if (curve.IsBar || curve is ErrorBarItem || curve is HiLowBarItem
                                    || curve is OHLCBarItem || curve is JapaneseCandleStickItem)
                                {
                                    ...
                                    if (isXBaseAxis)
                                    {
                                        ...
                                        if (xAct < centerVal - barWidthUserHalf || xAct > centerVal + barWidthUserHalf
                                    ...
                                    else
                                    {
                                        ...
                                        if (yAct < centerVal - barWidthUserHalf || yAct > centerVal + barWidthUserHalf
                                ...
                                else if (xVal >= xAxis._scale._min && xVal <= xAxis._scale._max && yVal >= yMinAct
                                         && yVal <= yMaxAct)
                                {
            ...
            {
                float halfSymbol = (float)(((LineItem)nearestCurve).Symbol.Size * this.CalcScaleFactor() / 2);
            ...
            {
                // otherwise, no valid point found
                return false;
            }
        }
