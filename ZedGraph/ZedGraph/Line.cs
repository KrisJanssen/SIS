// --------------------------------------------------------------------------------------------------------------------
// <copyright company="" file="Line.cs">
//   
// </copyright>
// <summary>
...
namespace ZedGraph.ZedGraph
{
    ...
    public class Line : LineBase, ICloneable, ISerializable
    {
        ...
        /// </param>
        /// <param name="curve">
        /// A <see cref="LineItem"/> representing this
        /// curve.
        /// </param>
        /// <param name="arrPoints">
        /// An array of <see cref="PointF"/> values in pixel
        ...
        public bool BuildLowPointsArray(GraphPane pane, CurveItem curve, out PointF[] arrPoints, out int count)
        {
            ...
            if (this.IsVisible && !this.Color.IsEmpty && points != null)
            {
                ...
                arrPoints =
                    new PointF[
                        (this._stepType == global::ZedGraph.ZedGraph.StepType.NonStep ? 1 : 2)
                        * (pane.LineType == LineType.Stack ? 2 : 1) * points.Count + 1];
        ...
        public bool BuildPointsArray(GraphPane pane, CurveItem curve, out PointF[] arrPoints, out int count)
        {
            ...
            if (this.IsVisible && !this.Color.IsEmpty && points != null)
            {
                ...
                // room to close out the curve for area fills.
                arrPoints =
                    new PointF[(this._stepType == global::ZedGraph.ZedGraph.StepType.NonStep ? 1 : 2) * points.Count + 1
                        ];
        ...
        /// </param>
        /// <param name="arrPoints">
        /// An array of <see cref="PointF"/> values in screen pixel
        /// coordinates representing the current curve.
        /// </param>
        /// <param name="count">
        /// The number of points contained in the "arrPoints"
                        ...
                        else
                        {
                            ...
                            if (!lastBad)
                            {
                                try
                                {
                                    ...
                                    if (lastX > 5000000 || lastX < -5000000 || lastY > 5000000 || lastY < -5000000
                                        || tmpX > 5000000 || tmpX < -5000000 || tmpY > 5000000 || tmpY < -5000000)
                                    {
        ...
        public void DrawSmoothFilledCurve(Graphics g, GraphPane pane, CurveItem curve, float scaleFactor)
        {
            ...
            if (this.IsVisible && !this.Color.IsEmpty && points != null
                && this.BuildPointsArray(pane, curve, out arrPoints, out count) && count > 2)
            {
                float tension = this._isSmooth ? this._smoothTension : 0f;
        ...
        /// </param>
        /// <param name="curve">
        /// A <see cref="CurveItem"/> representing this
        /// curve.
        /// </param>
        /// <param name="scaleFactor">
        /// The scaling factor to be used for rendering objects.  This is calculated and
